...
def _image_hf_sync_status(row: dict) -> str:
    return str(_get_first(row, "hf_sync_status", "hfSyncStatus") or "")


def _image_storage_status(row: dict) -> str:
    return str(_get_first(row, "storage_status", "storageStatus") or "")
...
def _normalise_image_row(row: dict) -> dict:
    """Supabase returns snake_case columns; older code expects camelCase.
    Keep both aliases in-memory so downstream services do not miss values.
    """
...
def _is_image_row(file_row: dict) -> bool:
    file_row = _normalise_image_row(file_row)
    mime = str(_get_first(file_row, "mime_type", "mimeType") or "")
    name = _image_file_name(file_row).lower()
...
def _status_counts(file_list: list[dict], key: str) -> dict[str, int]:
    aliases = {
        "hfSyncStatus": ("hf_sync_status", "hfSyncStatus"),
        "hf_sync_status": ("hf_sync_status", "hfSyncStatus"),
        "storageStatus": ("storage_status", "storageStatus"),
        "storage_status": ("storage_status", "storageStatus"),
        "hfPath": ("hf_path", "hfPath"),
        "hf_path": ("hf_path", "hfPath"),
    }
...
def _vercel_remote_ready(file_row: dict) -> bool:
    file_row = _normalise_image_row(file_row)
    # In Vercel/remote mode, exact DB hf_path is enough to attempt download.
    # Status is useful for logs, but should not wrongly exclude valid images.
    return bool(_image_hf_path(file_row))
...
def _repair_remote_image_rows(project_id: str, dataset_id: str, file_list: list[dict]) -> int:
    ...
    for row in file_list:
        ...
        row["hf_path"] = row["hfPath"] = resolved_hf_path
        row["hf_sync_status"] = row["hfSyncStatus"] = "synced"
        row["storage_status"] = row["storageStatus"] = "remote_ready"
    ...
    if remote_image_mode:
        ...
        for f in db_file_list:
            ...
            if hf_path and not _image_hf_path(f):
                f["hf_path"] = f["hfPath"] = hf_path
                f["hf_sync_status"] = f["hfSyncStatus"] = "synced"
                f["storage_status"] = f["storageStatus"] = "remote_ready"
                repaired += 1
        if repaired:
